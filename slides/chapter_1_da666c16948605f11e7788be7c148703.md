...
`@script`
Lets begin by applying the AR model to the Johnson&Johnson dataset.

First, let's load and view the JJ data in the pandas data frame using this block of code. The JJ data has two columns; the quarter dates for 20 years and next for returns realised in dollars. We load the data using pandas and convert the date column to timeseries object followed by setting it as index. So the head of data looks as shown here.
...
`@script`
Now let's apply the AR model available in the statsmodel package of python by importing statsmodel.tsa.ar_model.

We extract the return values of JJ-data and split it into train and test set. The last 8 quarters is our testset. Next we initialise the AR object on the train data. 

In the next block of code we will try to understand how to find the significant time lags to be used for our train set.There are the two parameters (maxlag and ic) which govern what will be the allowed number of time lags for the model.

First, here is the default internal lag value calculation inside the AR, in case when the two parameters are missing. Essentially time lag is directly proportional to the nobs, so more observations means more significant time lags that will be added to the model. Here 11 is the default value.

Next we can also specify the maxlag parameter as seen here.

Finally we can let the model fit the data for all the maxlag between 1 to 8, and based on the error measure aic we get 5 as significant number of time lags.
