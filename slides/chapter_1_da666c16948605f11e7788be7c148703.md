...
The mathematical notation of such process can be written as shown here. Where Xt is the value at instant time "t" and phi is the weights that the model will predict that is to be multiplied with the previous time lag values and Et is the error term. Essentially, we have "a sum of weighted previous time lags" to forecast the future values. 
...
Do note that phi is always between -1 and 1 meaning a part of the past signal is retained in the future instances only.
...
`@script`
Remember we learnt Auto correlated function, to calculate the correlation b/w the time lags at specific points t,t+1,t+2 and so on wrt "t". We can use the ACF to help us estimate the significant time lags to be considered for AR model. 
