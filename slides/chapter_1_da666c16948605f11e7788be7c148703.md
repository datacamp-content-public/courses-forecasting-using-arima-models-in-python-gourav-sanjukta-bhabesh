...
We extract the return values of dataframe and split it to train and test set here. The last two years or 8 quarters is our testset. Next we initialize the AR object on the train data. 
...
Finally we can let the model fit the data for all the values between 1 to maxlag=8 are tried, and based on error measure such as "Akaike Information Criterion" we get 5 as significant number of time lags.
