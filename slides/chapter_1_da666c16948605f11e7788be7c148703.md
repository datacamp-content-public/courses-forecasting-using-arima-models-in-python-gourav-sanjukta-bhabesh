...
`@script`
Using the model_fit that we trained earlier we can predict the values as shown here. Since these values are sequentially ordered we pass the start as the length of train and end as length of train set plus test set. Using the mean_squared_error measure we get 0.6 which is fairly a very good prediction.
We can also print the weights or phi in our model equation using the model_fit.params command. Do note that phi is always between -1 and 1 meaning a part of the past signal is retained in the future instances only.


---
## Plot comparing the predicted and actual quarterly returns for the year 1979 and 1980
...
`@script`
Here is the plot for the same and it seems the model has done a good job in learning the trend from the past returns!!
...
`@script`
Remember we learnt ACF(Auto correlated function), to calculate the correlation between the time lags at a specific point say "t" wrt t,t+1,t+2 and so on. We can use the ACF to help us estimate the significant time lags to be considered for AR model. 

Lets try this approach here and estimate the time lags for JJ data prediction. We can use the plot_acf from statsmodels package to plot the acf for the JJ data set. 
We will print the acf both from the actual data and the predicted data from the AR model.
1) First we pass the data frame to plot_acf specifying the lags to be considered, (here I have supplied 12). 
2) Second we pass the model_fit.predict values.
Then I have summarised the output in a dataframe called ACF_df as shown here.

We can see the correlation falls below 0.50 around 11 timelags for the actual dataset. And for fitted model it falls quickly as model we built is an approximation of the actual data.
